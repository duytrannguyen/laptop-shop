...
export default function ProductsManage() {
  ...
  const handleGalleryFiles = async (e) => {
    ...
    const newFiles = files.filter(f => f.size <= 5 * 1024 * 1024).map(file => ({
      file,
          ...
          <div className="admin-form-grid">
            ...
            <div className="admin-form-right">
              ...
              <div className="admin-form-section">
                ...
                {(form.image || selectedImagePreview) && (
                    <button type="button" className="btn btn-sm" style={{ marginTop: '10px', width: '100%', background: '#f1f3f5', color: '#000', border: '1px solid #ddd', fontWeight: 'bold' }} onClick={() => { setForm(prev => ({ ...prev, image: '' })); setSelectedImageFile(null); setSelectedImagePreview(null); }}>Xóa ảnh đại diện</button>
                )}
              </div>
