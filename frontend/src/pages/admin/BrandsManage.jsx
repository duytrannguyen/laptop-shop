...
export default function BrandsManage() {
  ...
  const load = () => {
    http.get('/admin/brands').then((r) => setData(r.data)).catch(() => {});
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editId) {
        await http.put(`/admin/brands/${editId}`, form);
      } else {
        await http.post('/admin/brands', form);
      }
      setModal(false);
      load();
    } catch (err) {
  ...
  const handleDelete = async (id, name) => {
    ...
    try {
      ...
      showToast(err.response?.data?.message || 'Không thể xóa thương hiệu');
    }
  };
  ...
  return (
    <>
      <div className="admin-header">
        <h1><FolderTree size={24} style={{ verticalAlign: 'middle', marginRight: '8px' }} />Quản lý thương hiệu</h1>
      ...
      <div className="admin-table-wrap">
        <div className="admin-table-header">
          <h3>Danh sách thương hiệu ({data.length})</h3>
        </div>
        ...
        <div className="admin-modal-overlay" onClick={() => setModal(false)}>
          <div className="admin-modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '480px' }}>
            <div className="admin-modal-header">
              <h2>{editId ? 'Sửa thương hiệu' : 'Thêm thương hiệu mới'}</h2>
            ...
            <form onSubmit={handleSave}>
              <div className="admin-modal-body">
                <div className="form-group">
                  ...
                  <input className="form-input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value, slug: toSlug(e.target.value) })} required placeholder="VD: Apple" />
                </div>
                <div className="form-group">
                  <label className="form-label">Slug *</label>
                  <input className="form-input" value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value })} required placeholder="apple" />
                </div>
