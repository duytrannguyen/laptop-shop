...
export default function CategoriesManage() {
  ...
  const load = () => {
    http.get('/admin/categories').then((r) => setData(r.data)).catch(() => {});
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editId) {
        await http.put(`/admin/categories/${editId}`, form);
      } else {
    ...
    <>
      ...
      <div className="admin-table-wrap">
        <div className="admin-table-header">
          <h3>Danh sách danh mục ({data.length})</h3>
        </div>
            ...
            <form onSubmit={handleSave}>
              <div className="admin-modal-body">
                <div className="form-group">
                  ...
                  <input className="form-input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value, slug: toSlug(e.target.value) })} required placeholder="VD: Điện thoại Apple" />
                </div>
                <div className="form-group">
                  <label className="form-label">Slug *</label>
                  <input className="form-input" value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value })} required placeholder="laptop-dell" />
                </div>
