...
export default function ProductGroupsManage() {
  ...
  const load = () => {
    http.get('/admin/product-groups').then((r) => setData(r.data)).catch(() => {});
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editId) {
        await http.put(`/admin/product-groups/${editId}`, form);
      } else {
        await http.post('/admin/product-groups', form);
      }
      setModal(false);
      load();
    } catch (err) {
  ...
  const handleDelete = async (id, name) => {
    if (!window.confirm(`Xác nhận xóa nhóm sản phẩm "${name}"?`)) return;
    try {
      await http.delete(`/admin/product-groups/${id}`);
      load();
    } catch (err) {
      showToast(err.response?.data?.message || 'Không thể xóa nhóm sản phẩm');
    }
  };
  ...
  return (
    <>
      <div className="admin-header">
        <h1><FolderTree size={24} style={{ verticalAlign: 'middle', marginRight: '8px' }} />Quản lý nhóm sản phẩm</h1>
        <div className="admin-header-actions">
          <button className="btn btn-primary btn-sm" onClick={openAdd}>
            <Plus size={16} /> Thêm nhóm sản phẩm
          </button>
        </div>
      </div>
      ...
      <div className="admin-table-wrap">
        <div className="admin-table-header">
          <h3>Danh sách nhóm sản phẩm ({data.length})</h3>
        </div>
        ...
        <div className="admin-modal-overlay" onClick={() => setModal(false)}>
          <div className="admin-modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '480px' }}>
            <div className="admin-modal-header">
              <h2>{editId ? 'Sửa nhóm sản phẩm' : 'Thêm nhóm sản phẩm mới'}</h2>
              <button className="admin-modal-close" onClick={() => setModal(false)}><X size={18} /></button>
            </div>
            <form onSubmit={handleSave}>
              <div className="admin-modal-body">
                <div className="form-group">
                  <label className="form-label">Tên nhóm sản phẩm *</label>
                  <input className="form-input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value, slug: toSlug(e.target.value) })} required placeholder="VD: MacBook" />
                </div>
                <div className="form-group">
                  <label className="form-label">Slug *</label>
                  <input className="form-input" value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value })} required placeholder="macbook" />
                </div>
